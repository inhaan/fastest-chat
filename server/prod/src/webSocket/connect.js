...
import { createNewChatRoom, getChatRoom, hasChatRoom, } from '../global/chatRooms.js';
...
/** WebSocket 업그레이드 */
export const handleWebSocketUpgrade = (request, socket, head) => {
    // URL에서 채팅방 ID 추출
    const { pathname, searchParams } = new URL(request.url, 'http://baseurl');
    const chatRoomId = decodeURIComponent(pathname.replace(/^\/chat\//gi, '') ?? '');
